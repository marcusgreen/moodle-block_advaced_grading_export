...
class block_advanced_grading_export_edit_form extends block_edit_form {

    protected function specific_definition($mform) {
        global $DB, $SESSION;
        $SESSION->block_advanced_grading_export = new stdClass();
        $SESSION->block_advanced_grading_export->status = 'defined';
        // Fields for editing HTML block title and contents.
        $mform->addElement('static', 'generalhelp', get_string('pluginname', 'block_advanced_grading_export') .
            ' ('.get_string('help').')' );
        $mform->addHelpButton('generalhelp', 'pluginname', 'block_advanced_grading_export');
        ...
        if (!$glossaries) {
            $mform->addElement('header', 'config_noglossaries', get_string('noglossaries', 'block_advanced_grading_export'));
        } else {
            ...
            foreach ($glossaries as $key => $value) {
                ...
                if ($numentries) {
                    $categoriesarray[$glossarystring][$key.',0'] = $glossarystring.' * '.
                        get_string('allentries', 'block_advanced_grading_export').' ('.$numentries.')';
            ...
            if ($totalnumentries === 0) {
                ...
                $mform->addElement('header', 'configheader', get_string($emptyglossaries, 'block_advanced_grading_export'));
            } else {
                ...
                $mform->addGroup($group, 'selectglossary',
                                get_string('selectglossary', 'block_advanced_grading_export'), '', false);

                // And select sortorder types to put in dropdown box.
                $mform->addHelpButton('selectglossary', 'selectglossary', 'block_advanced_grading_export');
                $types = array(
                    0 => get_string('concept', 'block_advanced_grading_export'),
                    1 => get_string('lastmodified', 'block_advanced_grading_export'),
                    2 => get_string('firstmodified', 'block_advanced_grading_export'),
                    3 => get_string('random', 'block_advanced_grading_export')
                );
                $mform->addElement('select', 'config_sortingorder',
                                get_string('sortingorder', 'block_advanced_grading_export'), $types);
                $mform->addHelpButton('config_sortingorder', 'sortingorder', 'block_advanced_grading_export');
                $mform->addElement('text', 'config_limitnum',
                                get_string('limitnum', 'block_advanced_grading_export'), array('size' => 5));
                $mform->addHelpButton('config_limitnum', 'limitnum', 'block_advanced_grading_export');
                ...
                $types = array(
                    0 => get_string('multichoice', 'block_advanced_grading_export').
                                ' ('.get_string('answernumberingabc', 'qtype_multichoice').')',
                    1 => get_string('multichoice', 'block_advanced_grading_export').' ('.
                                get_string('answernumberingABCD', 'qtype_multichoice').')',
                    2 => get_string('multichoice', 'block_advanced_grading_export').' ('.
                                get_string('answernumbering123', 'qtype_multichoice').')',
                    3 => get_string('multichoice', 'block_advanced_grading_export').' ('.
                                get_string('answernumberingnone', 'qtype_multichoice').')',
                    4 => get_string('shortanswer_0', 'block_advanced_grading_export'),
                    5 => get_string('shortanswer_1', 'block_advanced_grading_export')
                );
                $mform->addElement('select', 'config_questiontype',
                                get_string('questiontype', 'block_advanced_grading_export'), $types);
                $mform->addHelpButton('config_questiontype', 'questiontype', 'block_advanced_grading_export');
            }
        }
    }
