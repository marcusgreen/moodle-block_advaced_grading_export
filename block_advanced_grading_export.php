 ...
 */
include ('classes/btec_grading_export.php');
class block_advanced_grading_export extends block_base {
    public function init() {
        global $SESSION;
        $this->title = get_string('pluginname', 'block_advanced_grading_export');
        //$grading_method = new  btec_grading_export();
        //exit();
        /*select * from mdl_grading_areas ga
join mdl_gradingform_rubric_criteria grc on grc.definitionid=ga.id
join mdl_gradingform_rubric_levels grl on grl.criterionid=grc.id */
    ...
    public function get_content() {
        ...
        if (empty($this->config->glossary) || empty($SESSION->block_advanced_grading_export->status) ) {
            if ($editing) {
                    $this->content->text = get_string('notyetconfiguredediting', 'block_advanced_grading_export');
            } else {
                $this->content->text = get_string('notyetconfigured', 'block_advanced_grading_export');
            }
        ...
        if (isset ($categories[1]) && $categories[1] != 0) {
            ...
            $categoryname = '<b>'.get_string('category', 'glossary').'</b>: '.
                get_string('allentries', 'block_advanced_grading_export');
        }
        ...
        $strnumentries = '<br />'.get_string('numentries', 'block_advanced_grading_export', $numentries);

        $sortorder = $this->config->sortingorder;
        $type[0] = get_string('concept', 'block_advanced_grading_export');
        $type[1] = get_string('lastmodified', 'block_advanced_grading_export');
        $type[2] = get_string('firstmodified', 'block_advanced_grading_export');
        $type[3] = get_string('random', 'block_advanced_grading_export');
        ...
        $stractualquestiontype = get_string($actualquestiontype, 'block_advanced_grading_export');
        $strsortorder = '<b>'.get_string('sortingorder', 'block_advanced_grading_export').'</b>: '.$type[$sortorder];
        $strquestiontype = '<b>'.get_string('questiontype', 'block_advanced_grading_export').'</b> '.$stractualquestiontype;
        $cm = get_coursemodule_from_instance("glossary", $glossaryid);
        $cmid = $cm->id;
        $glosssaryname = "<em>$cm->name</em>";
        $title = get_string('clicktoexport', 'block_advanced_grading_export');
